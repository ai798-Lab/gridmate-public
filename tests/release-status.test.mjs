...
import { isDownloadReady, isPublicLink, PUBLIC_WEBSITE, websiteDownloadURL } from '../release-status.mjs';

const valid = { schemaVersion:1, version:'0.3.0', status:'published', minimumMacOS:'13.0', architecture:'arm64', downloadUrl:`${PUBLIC_WEBSITE}/downloads/SnapTiler-0.3.0-arm64.dmg`, releaseUrl:`${PUBLIC_WEBSITE}/releases/0.3.0.html`, sha256:'a'.repeat(64), sizeBytes:1000, releasedAt:'2026-09-17', appleNotarized:true, gatekeeperAccepted:true, downloadedArtifactTested:true };
test('same-domain download retains all release gates and exact version', () => {
  const websiteDownloadUrl = 'https://snaptiler.com/downloads/SnapTiler-0.3.0-arm64.dmg';
  ...
  for (const url of ['https://snaptiler.com.evil.test/downloads/SnapTiler-0.3.0-arm64.dmg','https://snaptiler.com/downloads/SnapTiler-0.2.0-arm64.dmg','https://snaptiler.com/downloads/SnapTiler-0.3.0-arm64.dmg?token=secret','javascript:alert(1)']) assert.equal(websiteDownloadURL({...valid,websiteDownloadUrl:url}),valid.downloadUrl);
});
...
test('pending release is unavailable', async () => {
  const manifest = JSON.parse(await readFile(new URL('../latest.json', import.meta.url), 'utf8'));
...
test('URLs must match the exact versioned public release', () => {
  for (const downloadUrl of ['javascript:alert(1)', 'file:///tmp/SnapTiler.dmg', 'https://example.com/app.dmg', `${PUBLIC_WEBSITE}/downloads/SnapTiler-0.2.0-arm64.dmg`, `${valid.downloadUrl}?token=private`]) assert.equal(isDownloadReady({ ...valid, downloadUrl }), false);
});
...
test('support and release URLs do not leak into private or lookalike repositories', () => {
  assert.equal(isPublicLink(`${PUBLIC_WEBSITE}/support.html`, 'issues'), true);
  for (const url of ['https://github.com/ai798-Lab/SnapTiler/issues', `${PUBLIC_WEBSITE}/support.html-evil`, `${PUBLIC_WEBSITE}/support.html?secret=x`, 'javascript:alert(1)', null]) assert.equal(isPublicLink(url, 'issues'), false);
});
